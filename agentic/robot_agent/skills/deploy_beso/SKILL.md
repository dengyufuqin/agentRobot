---
name: deploy_beso
description: "Deploy beso model as a WebSocket policy server (local or remote via SSH)"
version: 2.0.0
category: deploy
parameters:
  node:
...
timeout: 120
command_template: |
  REPO=$AGENTROBOT_ROOT/beso
  mkdir -p $AGENTROBOT_ROOT/logs

  if [ -z "{node}" ] || [ "{node}" = "localhost" ]; then
    # === Local mode ===
    LOG=$AGENTROBOT_ROOT/logs/beso-${HOSTNAME:-local}-{port}.log
    echo "Deploying beso locally (GPU {gpu_id}, port {port})..."
    ...
    # === Remote mode via SSH ===
    LOG=$AGENTROBOT_ROOT/logs/beso-{node}-{port}.log
  ...
  echo "Wait ~90s for model to load."
---

# Deploy beso Policy Server
