...
#SBATCH --output=/mnt/vast/home/yd66byne/code/agentRobot/logs/eval-ovla-ms2-%j.log

# ============================================================
#  OpenVLA-OFT × ManiSkill (7 tasks) — v10: pd_ee_delta_pose
# ============================================================
...
echo "============================================"
echo "  OpenVLA-OFT × ManiSkill (v10 — pd_ee_delta_pose)"
...
CUDA_VISIBLE_DEVICES=0 PYTHONPATH=$AGENTROBOT_ROOT/agentic/policy_websocket/src:$AGENTROBOT_ROOT/openvla \
  ...
  --pretrained_checkpoint moojink/openvla-7b-oft-finetuned-libero-spatial --port $PORT &
PID=$!
...
for TASK in PickCube-v1 StackCube-v1 PushCube-v1 PegInsertionSide-v1 TurnFaucet-v1 LiftPegUpright-v1 PlugCharger-v1; do
  echo ""
  echo "=========================================="
  echo "  EVAL: OpenVLA on ManiSkill/$TASK ($(date))"
  echo "=========================================="
  ...
  echo "  → OpenVLA on ManiSkill/$TASK: exit $?"
done
