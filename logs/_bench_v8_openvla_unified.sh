...
#SBATCH --output=/mnt/vast/home/yd66byne/code/agentRobot/logs/eval-ovla-uni-%j.log

# ============================================================
#  OpenVLA-OFT (unified multi-suite) × LIBERO (4 suites) + ManiSkill (7 tasks)
# ============================================================
...
echo "============================================"
echo "  OpenVLA-OFT (unified) × LIBERO + ManiSkill (v8)"
...
for SUITE in libero_spatial libero_object libero_goal libero_10; do
  ...
  PYTHONPATH=$AGENTROBOT_ROOT/agentic/policy_websocket/src:$AGENTROBOT_ROOT/LIBERO \
    ...
    --policy_server_addr localhost:$PORT --policy openvla --benchmark_name "$SUITE" \
...
for TASK in PickCube-v1 StackCube-v1 PushCube-v1 PegInsertionSide-v1 TurnFaucet-v1 LiftPegUpright-v1 PlugCharger-v1; do
  echo ""
  echo "=========================================="
  echo "  EVAL: OpenVLA-unified on ManiSkill/$TASK ($(date))"
  echo "=========================================="
  PYTHONPATH=$AGENTROBOT_ROOT/agentic/policy_websocket/src:$AGENTROBOT_ROOT/ManiSkill \
    ...
    --num_trials 5 --no_save_video \
    --log_dir $AGENTROBOT_ROOT/logs/eval_results 2>&1
  echo "  → OpenVLA-unified on ManiSkill/$TASK: exit $?"
done
