...
#SBATCH --output=/mnt/vast/home/yd66byne/code/agentRobot/logs/eval-pi0-lib-%j.log

# ============================================================
#  pi0 × LIBERO (4 suites) + ManiSkill TurnFaucet rerun
# ============================================================
...
echo "============================================"
echo "  pi0 × LIBERO + ManiSkill TurnFaucet (v9)"
...
# === LIBERO Evaluations (correct arg: --task_suite_name) ===
echo ""
...
for SUITE in libero_spatial libero_object libero_goal libero_10; do
  echo ""
  echo "=========================================="
  echo "  EVAL: pi0 on LIBERO/$SUITE ($(date))"
  echo "=========================================="
  PYTHONPATH=$AGENTROBOT_ROOT/agentic/policy_websocket/src:$AGENTROBOT_ROOT/LIBERO \
    ...
    --policy_server_addr localhost:$PORT --policy pi0 --task_suite_name "$SUITE" \
    --num_trials_per_task 5 --no_save_video \
    --log_dir $AGENTROBOT_ROOT/logs/eval_results 2>&1
  echo "  → pi0 on LIBERO/$SUITE: exit $?"
done

# === ManiSkill TurnFaucet rerun (assets now pre-downloaded) ===
echo ""
echo "=== ManiSkill TurnFaucet Rerun ==="
echo ""
echo "=========================================="
echo "  EVAL: pi0 on ManiSkill/TurnFaucet-v1 ($(date))"
echo "=========================================="
PYTHONPATH=$AGENTROBOT_ROOT/agentic/policy_websocket/src:$AGENTROBOT_ROOT/ManiSkill \
  ...
  --policy_server_addr localhost:$PORT --policy pi0 --env_id "TurnFaucet-v1" \
  --num_trials 5 --no_save_video \
  --log_dir $AGENTROBOT_ROOT/logs/eval_results 2>&1
echo "  → pi0 on ManiSkill/TurnFaucet-v1: exit $?"
