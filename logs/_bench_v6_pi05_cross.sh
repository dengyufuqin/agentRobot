...
#SBATCH --output=/mnt/vast/home/yd66byne/code/agentRobot/logs/eval-pi05-x-%j.log

# ============================================================
#  pi0.5 × ManiSkill + RoboTwin (cross-benchmark generality)
# ============================================================
...
# Wait for server (JIT warmup takes several minutes)
...
for TASK in stack_blocks_two handover_block pick_diverse_bottles; do
  echo ""
  echo "=========================================="
  echo "  EVAL: pi0.5 on RoboTwin/$TASK ($(date))"
  echo "=========================================="
  PYTHONPATH=$AGENTROBOT_ROOT/agentic/policy_websocket/src \
    ...
    --num_trials 5 --no_save_video \
    --log_dir $AGENTROBOT_ROOT/logs/eval_results 2>&1
  echo "  → pi0.5 on RoboTwin/$TASK: exit $?"
done
